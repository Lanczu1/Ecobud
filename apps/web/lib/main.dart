                    ...
                    child: Column(
                      children: [
                        ...
                        _RevealOnBuild(
                          ...
                          child: _AdminPortalSection(key: _adminKey),
                        ),
                      ],
                    ),
                  ),
...
class _AdminPortalSection extends StatefulWidget {
  const _AdminPortalSection({super.key});

  @override
  State<_AdminPortalSection> createState() => _AdminPortalSectionState();
}

class _AdminPortalSectionState extends State<_AdminPortalSection> {
  final _emailController = TextEditingController(text: 'admin@ecobud.app');
  final _passwordController = TextEditingController(text: 'admin12345');
  final _lessonTitleController =
      TextEditingController(text: 'Community Gardening Basics');
  final _lessonSummaryController = TextEditingController(
      text:
          'Teach members how to start neighborhood gardens and compost responsibly.');
  final _lessonContentController = TextEditingController(
      text:
          'Start with a compost-ready soil mix, pick resilient vegetables, assign community watering schedules, and track progress weekly in ECOBUD.');
  final _lessonCategoryController =
      TextEditingController(text: 'Community Action');
  final _lessonImageController = TextEditingController(
      text:
          'https://images.unsplash.com/photo-1466692476868-aef1dfb1e735?auto=format&fit=crop&w=1200&q=80');
  final _lessonDurationController = TextEditingController(text: '12');
  final _lessonRatingController = TextEditingController(text: '4.8');
  final _lessonPointsController = TextEditingController(text: '18');

  final _challengeTitleController =
      TextEditingController(text: 'Neighborhood Clean Street Week');
  final _challengeDescriptionController = TextEditingController(
      text:
          'Organize a seven-day effort to reduce litter, sort recyclables, and share before/after proof.');
  final _challengeCategoryController = TextEditingController(text: 'Community');
  final _challengeDurationController = TextEditingController(text: '7');
  final _challengePointsController = TextEditingController(text: '35');
  final _challengeImageController = TextEditingController(
      text:
          'https://images.unsplash.com/photo-1528323273322-d81458248d40?auto=format&fit=crop&w=1200&q=80');
  final _challengeBadgeController = TextEditingController(text: 'Street Hero');

  final _eventTitleController =
      TextEditingController(text: 'Riverbank Recycling Rally');
  final _eventDescriptionController = TextEditingController(
      text:
          'Meet local volunteers, sort recyclables, and log collective impact on the ECOBUD transparency feed.');
  final _eventLocationController =
      TextEditingController(text: 'Pasig River Walk');
  final _eventDateController =
      TextEditingController(text: '2026-05-18T09:00:00.000Z');
  final _eventCapacityController = TextEditingController(text: '120');
  final _eventPointsController = TextEditingController(text: '40');
  final _eventImageController = TextEditingController(
      text:
          'https://images.unsplash.com/photo-1488521787991-ed7bbaae773c?auto=format&fit=crop&w=1200&q=80');
  final _eventLatitudeController = TextEditingController(text: '14.5995');
  final _eventLongitudeController = TextEditingController(text: '120.9842');

  String? _token;
  String? _adminError;
  bool _isBusy = false;
  String _busyMessage = 'Preparing the admin console...';
  bool _lessonFeatured = true;
  ...
  bool get _isAuthenticated => _token != null;
  ...
  Future<Map<String, dynamic>> _request(
    ...
    final request = http.Request(method, uri)
      ..headers.addAll(_jsonHeaders(_token))
    ...
    final payload = jsonDecode(httpResponse.body) as Map<String, dynamic>;

    if (httpResponse.statusCode < 200 || httpResponse.statusCode >= 300) {
      throw Exception(payload['message'] ?? 'Unexpected API error.');
    }
  ...
  Future<T> _runBusyAction<T>(
    String message,
    Future<T> Function() action, {
    Duration minimumDuration = const Duration(milliseconds: 720),
  }) async {
    final startedAt = DateTime.now();
    if (mounted) {
      setState(() {
  ...
  Future<void> _runBusyTransition(
    String message,
    VoidCallback action, {
    Duration minimumDuration = const Duration(milliseconds: 460),
  }) async {
  ...
  Future<void> _handleLogin() async {
    await _runBusyAction('Unlocking the admin portal...', () async {
      setState(() {
        _adminError = null;
      });
      ...
      try {
        ...
        if (response.statusCode < 200 || response.statusCode >= 300) {
          throw Exception(payload['message'] ?? 'Invalid credentials.');
        }
        ...
        setState(() {});
        _showSnack('Admin portal unlocked.');
      } catch (error) {
        setState(() {
          _adminError = error.toString().replaceFirst('Exception: ', '');
        });
      }
    }, minimumDuration: const Duration(milliseconds: 860));
  }
  ...
  Future<void> _hydrateAdminData() async {
    ...
    final results = await Future.wait([
      _request('/admin/dashboard'),
      _request('/admin/users'),
      _request('/admin/lessons'),
      _request('/admin/challenges'),
      _request('/admin/events'),
    ]);
    ...
    setState(() {
      _totals = results[0]['totals'] as Map<String, dynamic>;
      _users = List<Map<String, dynamic>>.from(results[1]['items'] as List);
      _lessons = List<Map<String, dynamic>>.from(results[2]['items'] as List);
      _challenges =
          List<Map<String, dynamic>>.from(results[3]['items'] as List);
      _events = List<Map<String, dynamic>>.from(results[4]['items'] as List);
    });
  }

  Future<void> _loadAdminData() async {
    await _runBusyAction('Refreshing admin data...', () async {
      await _hydrateAdminData();
    });
  }

  Future<void> _createLesson() async {
    await _runBusyAction('Publishing the new lesson...', () async {
      try {
        await _request(
          '/admin/lessons',
          method: 'POST',
          body: {
            ...
            'summary': _lessonSummaryController.text.trim(),
            'content': _lessonContentController.text.trim(),
            'category': _lessonCategoryController.text.trim(),
            'imageUrl': _lessonImageController.text.trim(),
            'durationMinutes': int.parse(_lessonDurationController.text),
            'rating': double.parse(_lessonRatingController.text),
            'pointsReward': int.parse(_lessonPointsController.text),
            'featured': _lessonFeatured,
          },
        );
        await _hydrateAdminData();
        _showSnack('Lesson created.');
      } catch (error) {
  ...
  Future<void> _createChallenge() async {
    await _runBusyAction('Launching the new challenge...', () async {
      try {
        await _request(
          '/admin/challenges',
          method: 'POST',
          body: {
            ...
            'category': _challengeCategoryController.text.trim(),
            'durationDays': int.parse(_challengeDurationController.text),
            'pointsReward': int.parse(_challengePointsController.text),
            'imageUrl': _challengeImageController.text.trim(),
            'badgeLabel': _challengeBadgeController.text.trim(),
  ...
  Future<void> _createEvent() async {
    await _runBusyAction('Creating the community event...', () async {
      try {
        await _request(
          '/admin/events',
          method: 'POST',
          body: {
            ...
            'pointsReward': int.parse(_eventPointsController.text),
            'imageUrl': _eventImageController.text.trim(),
            'latitude': double.parse(_eventLatitudeController.text),
            'longitude': double.parse(_eventLongitudeController.text),
          },
        );
        await _hydrateAdminData();
  ...
  Future<void> _deleteResource(String resource, String id) async {
    await _runBusyAction(
        'Removing ${resource.substring(0, resource.length - 1)}...', () async {
      try {
        await _request('/admin/$resource/$id', method: 'DELETE');
        await _hydrateAdminData();
        _showSnack(
            '${resource[0].toUpperCase()}${resource.substring(1, resource.length - 1)} deleted.');
      } catch (error) {
        _showSnack(error.toString().replaceFirst('Exception: ', ''));
      }
    });
  ...
  Future<void> _markAttendance(String eventId, String registrationId) async {
    await _runBusyAction('Verifying attendance and rewards...', () async {
      try {
        await _request(
            '/admin/events/$eventId/registrations/$registrationId/attend',
            method: 'POST');
        await _hydrateAdminData();
        _showSnack('Attendance verified and rewarded.');
      } catch (error) {
        _showSnack(error.toString().replaceFirst('Exception: ', ''));
      }
    });
  ...
  Widget _buildOverviewPanel() {
    final totals = _totals ?? const <String, dynamic>{};
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Wrap(
        ...
        _AdminPanel(
          ...
          subtitle:
              'Latest users ordered by points so moderators can spot top contributors.',
          child: Column(
            children: _users
                .take(10)
                .map(
  ...
  Widget build(BuildContext context) {
    ...
    return Stack(
      children: [
            ...
            children: [
                ...
                children: [
                  ...
                  Expanded(
                    child: Column(
                      ...
                      children: const [
                        ...
                        SizedBox(height: 4),
                        Text(
                          'Create content, monitor users, and verify community event attendance from one dashboard.',
                          style:
                              TextStyle(color: Color(0xFF6B7280), height: 1.5),
                        ),
                      ],
                    ),
                  ),
                  if (_isAuthenticated)
                    Wrap(
                      ...
                      children: [
                        OutlinedButton.icon(
                          onPressed: _isBusy ? null : () => _loadAdminData(),
                        ...
                        _GradientButton(
                          onPressed: _isBusy
                              ...
                              : () {
                                  _runBusyTransition(
                                    'Signing out of the admin portal...',
                                    () => setState(() {
                                      _token = null;
                                      _totals = null;
                                      _users = const [];
              ...
              if (!_isAuthenticated)
                _AdminPanel(
                  title: 'Authenticate',
                  subtitle:
                      'Use the seeded admin account to unlock lesson, challenge, and event management.',
                  child: Column(
                    children: [
                      if (_adminError != null)
                        Container(
                          ...
                          decoration: BoxDecoration(
                            ...
                            borderRadius: BorderRadius.circular(16),
                            border: Border.all(color: const Color(0xFFF4B8B3)),
                          ),
                          child: Text(_adminError!,
                ...
                Wrap(
                  ...
                  children: [
                    ...
                    _AdminTabButton(
                        label: 'Lessons',
                        active: _activePanel == 1,
                        onTap: () {
                          _runBusyTransition(
                            'Opening lessons...',
                            () => setState(() => _activePanel = 1),
                          );
                        }),
                    _AdminTabButton(
                        label: 'Challenges',
                        active: _activePanel == 2,
                        onTap: () {
                          _runBusyTransition(
                            'Opening challenges...',
                    ...
                    _AdminTabButton(
                        label: 'Events',
                        active: _activePanel == 3,
                        onTap: () {
                          _runBusyTransition(
                            'Opening events...',
                            () => setState(() => _activePanel = 3),
                          );
                        }),
                  ],
                ),
                const SizedBox(height: 22),
                if (_activePanel == 0) _buildOverviewPanel(),
                if (_activePanel == 1)
                  _AdminManagerPanel(
                    title: 'Lessons',
                    subtitle:
                        'Publish new learning content for the mobile Learn & Grow screen.',
                    formChildren: [
                      _AdminField(
                          label: 'Title', controller: _lessonTitleController),
                      _AdminField(
                          label: 'Summary',
                      ...
                      _AdminField(
                          label: 'Image URL',
                          controller: _lessonImageController),
                      Wrap(
                        spacing: 16,
                        runSpacing: 16,
                        children: [
                          ...
                          SizedBox(
                              ...
                              child: _AdminField(
                                  label: 'Rating',
                                  controller: _lessonRatingController)),
                          SizedBox(
                              width: 160,
                              child: _AdminField(
                                  label: 'Points',
                                  controller: _lessonPointsController)),
                        ],
                      ),
                      SwitchListTile.adaptive(
                        contentPadding: EdgeInsets.zero,
                        value: _lessonFeatured,
                        onChanged: (value) =>
                            setState(() => _lessonFeatured = value),
                        title: const Text('Featured lesson'),
                      ),
                    ],
                    onCreate: _createLesson,
                    actionLabel: 'Create lesson',
                    items: _lessons,
                    itemBuilder: (lesson) => _AdminResourceTile(
                      title: lesson['title'] as String,
                      subtitle:
                          '${lesson['category']} • ${lesson['pointsReward']} pts',
                      onDelete: () =>
                          _deleteResource('lessons', lesson['id'] as String),
                    ),
                  ),
                if (_activePanel == 2)
                  _AdminManagerPanel(
                    title: 'Challenges',
                    subtitle:
                        'Ship new challenge campaigns tied to badges and point rewards.',
                    formChildren: [
                      _AdminField(
                          label: 'Title',
                      ...
                      Wrap(
                        ...
                        children: [
                          SizedBox(
                              ...
                              child: _AdminField(
                                  label: 'Category',
                                  controller: _challengeCategoryController)),
                          SizedBox(
                            width: 220,
                            child: DropdownButtonFormField<String>(
                              key: ValueKey(_challengeDifficulty),
                              initialValue: _challengeDifficulty,
                              decoration: _adminFieldDecoration('Difficulty'),
                              items: const [
                                DropdownMenuItem(
                              ...
                              onChanged: (value) => setState(
                                  () => _challengeDifficulty = value ?? 'EASY'),
                            ),
                          ),
                        ],
                      ),
                      Wrap(
                        ...
                        children: [
                          ...
                          SizedBox(
                              ...
                              child: _AdminField(
                                  label: 'Badge label',
                                  controller: _challengeBadgeController)),
                        ],
                      ),
                      _AdminField(
                          label: 'Image URL',
                          controller: _challengeImageController),
                      SwitchListTile.adaptive(
                        contentPadding: EdgeInsets.zero,
                        value: _challengeActive,
                        onChanged: (value) =>
                            setState(() => _challengeActive = value),
                        title: const Text('Challenge is active'),
                      ),
                    ],
                    onCreate: _createChallenge,
                    actionLabel: 'Create challenge',
                    items: _challenges,
                    itemBuilder: (challenge) => _AdminResourceTile(
                ...
                if (_activePanel == 3)
                  _AdminManagerPanel(
                    title: 'Events',
                    subtitle:
                        'Create eco events and verify attendance for automatic reward payout.',
                    formChildren: [
                      _AdminField(
                          label: 'Title', controller: _eventTitleController),
                      _AdminField(
                          label: 'Description',
                    ...
                    items: _events,
                    itemBuilder: (event) => _AdminEventTile(
                      event: event,
                      onDelete: () =>
                          _deleteResource('events', event['id'] as String),
        ...
        if (_isBusy)
          Positioned.fill(
            child: _SectionActionOverlay(message: _busyMessage),
          ),
      ],
    );
  }
...
InputDecoration _adminFieldDecoration(String label) => InputDecoration(
      labelText: label,
      filled: true,
      ...
      border: OutlineInputBorder(
        borderRadius: BorderRadius.circular(18),
        borderSide: const BorderSide(color: EcoBudPalette.border),
      ),
      ...
      focusedBorder: OutlineInputBorder(
        borderRadius: BorderRadius.circular(18),
        borderSide: const BorderSide(color: EcoBudPalette.emerald, width: 1.4),
      ),
    );
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          Text(subtitle,
              style:
                  const TextStyle(color: EcoBudPalette.textMuted, height: 1.5)),
          const SizedBox(height: 18),
          child,
        ],
      ),
...
class _AdminField extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return TextField(
      ...
      maxLines: maxLines,
      obscureText: obscure,
      decoration: _adminFieldDecoration(label),
    );
  }
}
...
class _AdminTabButton extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return TextButton(
      onPressed: onTap,
      style: TextButton.styleFrom(
        backgroundColor: active ? EcoBudPalette.emerald : EcoBudPalette.mist,
        foregroundColor: active ? Colors.white : EcoBudPalette.pine,
        padding: const EdgeInsets.symmetric(horizontal: 18, vertical: 14),
        textStyle:
            const TextStyle(fontWeight: FontWeight.w800, letterSpacing: 0.1),
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Icon(icon, color: EcoBudPalette.emerald),
          const SizedBox(height: 14),
          Text(value,
              style:
                  const TextStyle(fontSize: 30, fontWeight: FontWeight.w900)),
          const SizedBox(height: 6),
          Text(label, style: const TextStyle(color: EcoBudPalette.textMuted)),
        ],
      ),
    );
  }
...
class _AdminManagerPanel extends StatelessWidget {
  const _AdminManagerPanel({
    required this.title,
    required this.subtitle,
  ...
  Widget build(BuildContext context) {
    return Column(
      children: [
        ...
        _AdminPanel(
          title: '$title Library',
          subtitle: 'Existing records coming from the seeded ECOBUD API.',
  ...
  Widget build(BuildContext context) {
    return ListTile(
      ...
      trailing: IconButton(
        onPressed: onDelete,
        icon:
            const Icon(Icons.delete_outline_rounded, color: Color(0xFFB42318)),
      ),
    );
  }
}
...
class _AdminEventTile extends StatelessWidget {
  ...
  final VoidCallback onDelete;
  final Future<void> Function(String eventId, String registrationId)
      onMarkAttendance;

  @override
  Widget build(BuildContext context) {
    final registrations = List<Map<String, dynamic>>.from(
        event['registrations'] as List? ?? const []);
